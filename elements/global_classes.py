    ...
    def draw(self, screen):
        ...
        if self.text != "":
            ...
            for i in range(len(lines)):
                lines[i] = font.render(lines[i], True, (255, 255, 255))
                text_height += lines[i].get_height()
            l = len(lines)
            for i in range(len(lines)):
                screen.blit(
                    lines[i],
                    (
                        self.x + (self.width  / 2 - lines[i].get_width()  / 2),
                        self.y + (self.height / 2 + (text_height/len(lines) * 
                        (len(lines) // 2 * -1 + i))) - 
                        (lines[i].get_height()/2 if len(lines) == 1 else 0)
                    )
                )
