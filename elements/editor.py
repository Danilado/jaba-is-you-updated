...
class Editor(GameStrategy):
    ...
    def __init__(self, screen: pygame.Surface):
        ...
        self.pagination_buttons = [
            Button(settings.RESOLUTION[0] + int(17 * settings.WINDOW_SCALE),
                   ...
                   int(75 * settings.WINDOW_SCALE), int(20 * settings.WINDOW_SCALE), (0, 0, 0), IuiSettings(),
        ...
        for index, text in enumerate(button_objects_array):
            button_array.append(
                ObjectButton(x=int(settings.RESOLUTION[0] + (28 + 84 * (index % 2)) * settings.WINDOW_SCALE),
                             y=int((25 + 55 * (index - index % 2)) * settings.WINDOW_SCALE),
    ...
    def unresize(self):
        ...
        self.screen = pygame.display.set_mode((1600 * settings.WINDOW_SCALE, 900 * settings.WINDOW_SCALE))
    ...
    def draw(self, events: List[pygame.event.Event], delta_time_in_milliseconds: int) -> Optional[State]:
        ...
        indicators = [
            Button(settings.RESOLUTION[0] + int(17 * settings.WINDOW_SCALE),
                   ...
                   int(75 * settings.WINDOW_SCALE), int(75 * settings.WINDOW_SCALE), (0, 0, 0), IuiSettings(),
                   f"Obj\n{self.name}"),
            Button(settings.RESOLUTION[0] + int(101 * settings.WINDOW_SCALE),
                   settings.RESOLUTION[1] - int(192 * settings.WINDOW_SCALE),
                   int(75 * settings.WINDOW_SCALE), int(75 * settings.WINDOW_SCALE), (0, 0, 0), IuiSettings(),
        ...
        if self.show_grid:
            for i in range(int(settings.RESOLUTION[0]) // int(50 * settings.WINDOW_SCALE) + 1):  # Отрисовать сетку
            ...
            for i in range(int(settings.RESOLUTION[1]) // int(50 * settings.WINDOW_SCALE) + 1):
                pygame.draw.line(self.screen, (255, 255, 255),
                                 (0, i * 50 * settings.WINDOW_SCALE - (1 if i == 18 else 0)),
