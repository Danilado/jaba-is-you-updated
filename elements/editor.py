...
from elements.global_classes import Button, EuiSettings, IuiSettings
...
def save(state):
    with open(f"levels/level{len(os.listdir('levels/'))}.omegapog_map_file_type_MLG_1337_228_100500_69_420", 'w', encoding='utf-8') as file:
    ...
    try:
        ...
        while not left:
            indicators = [
                ...
                f"Tool\n{'Create' if tool == 1 else 'Delete' if tool == 0 else 'Lookup'}", partial(set_tool, 0 if tool == 1 else 1 if tool == 2 else 2)),
                Button(settings.resolution[0] + 101, settings.resolution[1] - 100, 75, 75, (0, 0, 0), IuiSettings(), 
                f"Dir\n{'↑' if direction == 0 else '→' if direction == 1 else '↓' if direction == 2 else '←'}", partial(turn, 1)),
            ]
                    ...
                    if focus[0] != -1 and focus[1] != -1:
                        ...
                        if tool == 1 and name != None:
                            ...
                            if not flag:
                                ...
                                current_state[focus[1]][focus[0]].append(Object(focus[0], focus[1], direction, name, None, is_text))
                        elif tool == 0:
...
name:           {name}
            ...
            for i in range(settings.resolution[1]//50+1):
                pygame.draw.line(main_screen, (255, 255, 255), (0, i*50 - (1 if i == 18 else 0)), (settings.resolution[0], i*50 - (1 if i == 18 else 0)), 1)  
